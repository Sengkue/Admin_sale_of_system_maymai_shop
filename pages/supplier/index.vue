<template>
  <div>
    ...
    <div v-if="getEdit">
      <SupplierEditSupplier />
    </div>
    <div v-if="getView">
      <SupplierViewSupplier />
    </div>
    <div v-if="getInsert">
      <SupplierInsertSupplier />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    getRow() {
      return this.$store.state.supplier.StateSelectAll.map((item, index) => {
        return {
          index: index + 1,
          ...item,
        };
    ...
    getEdit() {
      return this.$store.state.supplier.edit;
    },
    getView() {
      return this.$store.state.supplier.view;
    },
    getInsert() {
      return this.$store.state.supplier.insert;
    },
    getOneUser() {
  ...
  methods: {
      ...
      await this.$swal
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            this.loading = true;
            await this.$store.dispatch("supplier/delete", { deleteId });
            await this.$store.dispatch("supplier/selectAll");
    ...
    async viewDetail(item) {
      ...
      this.$store.commit("supplier/setView", true);
    ...
    async Insert() {
      this.isLoading = false;
      await this.$store.commit("supplier/setInsert", true);
      this.$store.dispatch('province/selectProvince')
    this.$store.dispatch('district/selectDistrict')
      this.isLoading = false;
    },
