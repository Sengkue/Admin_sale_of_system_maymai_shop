...
export default {
  data: () => ({
    ...
    discountRules: [
      ...
      (v) =>
        (!isNaN(parseFloat(v)) && isFinite(v)) || 'Discount must be a number',
    ],
    id:''
  }),
  computed: {
  ...
  created() {
    this.dialog = this.$store.state.promotion.edit
    if (this.getUser) {
      this.name = this.getUser.name
      this.condition = this.getUser.condition
      this.discount = this.getUser.discount
      this.id = this.getUser.id
    }
  },
  methods: {
    back() {
      this.$store.commit('promotion/setEdit', false)
    },
    async updateItem() {
      ...
      await this.$store.dispatch('promotion/update', { formData, id })
